...
private:
    ...
    void bind()
    {
        ...
        //Set TCP_NODELAY: Deactive Nagle, so that laser scans will be sent immediately.
        rc = setsockopt(bindHandle, IPPROTO_TCP, TCP_NODELAY, &optval, optlen);
        Error = errno;
        if (rc != 0) {
            throw std::runtime_error("BinaryInterfaceServer::bind: "
                    "Error setting TCP_NODELAY. Errno: " +
                    std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
