...
public:
    ...
    int32_t read(void * const target, const int32_t size)
    {
        ...
        {
            void * const targetBufferPtr {static_cast<void*>(static_cast<char*>(target) + bytesRead)};
...
private:
    ...
    void Connect()
    {
        ...
        struct addrinfo *res;
        {
            ...
            if (rc != 0)
            {
                throw std::runtime_error("BinaryInterfaceClient::Connect: Error resolving the given address and port as stream-socket. Errno: "
        ...
        } while ((rc < 0) && ((Error == ETIMEDOUT) || (Error == ENETUNREACH) || (Error == ECONNREFUSED)) && (Index++ < retryAttempts));
  
        freeaddrinfo(res);
        if (Handle == -1)
        {
            throw std::runtime_error("BinaryInterfaceClient::Connect: Error during connection. Last errno was: "
                    + std::to_string(Error)  + ": " + std::string(strerror(Error)));
        }
