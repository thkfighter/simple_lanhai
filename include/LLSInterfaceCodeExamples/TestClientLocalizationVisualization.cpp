/*
 * File: TestClientLocalizationVisualization.cpp
 ...
 */
#include "BinaryInterfaceLocalizationVisualizationStruct.h"
 ...
 * Connect to the TCP Port 9010 (default port of localization visualization interface)
 */
void RunTcpTest (const std::string &hostName)
{
    ...
    try
    {
        ...
        BinaryInterfaceClient client(hostName, 9010);

        LocalizationVisualizationMessage message;
        int32_t rv = 0;
        while (rv >= 0)
        {
            std::cout << "----------------- Localization Visualization Message -----------------\n";
            std::cout << "------------------ Read legacy block. Original Data ------------------\n";
            rv = client.read(&message.timestamp, sizeof(message.timestamp));
            checkReturnValue(rv);
            rv = client.read(&message.uniqueId, sizeof(message.uniqueId));
            checkReturnValue(rv);
            rv = client.read(&message.locState, sizeof(message.locState));
            checkReturnValue(rv);
            ...
            std::cout << "uniqueId = " << message.uniqueId << '\n';
            std::cout << "locState = " << message.locState << '\n';
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "TestClientLocalizationVisualization died with error!" << '\n';
