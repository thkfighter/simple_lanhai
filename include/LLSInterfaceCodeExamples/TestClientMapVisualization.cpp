/*
 * File: TestClientMapVisualization.cpp
 ...
 */
#include "BinaryInterfaceMapVisualizationStruct.h"
...
void checkReturnValue(int32_t returnValue)
{
    if (returnValue == 0)
    {
        ...
        std::cout << "Socket call __read__ returned error." << '\n';
    }
}

/*
 * Run Tcp Test -> This accepts a hostname and (optionally) a filename 
 * Connect to the TCP Port 9006 (default port of map visualization interface)
 */
void RunTcpTest (const std::string &hostName)
{
    ...
    try
    {
        ...
        BinaryInterfaceClient client(hostName, 9006);

        MapVisualizationMessage message;
        int32_t rv = 0;
        while (rv >= 0)
        {
            std::cout << "---- ------------- Map Visualization Message -------------------------\n";
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "TestClientMapVisualization died with error!" << '\n';
