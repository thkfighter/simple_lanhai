...
private:
    ...
    void Connect()
    {
        ...
        struct addrinfo *res;
        {
            ...
            if (rc != 0)
            {
                throw std::runtime_error("BinaryInterfaceClien::Connect: Error resolving the given address and port as stream-socket. Errno: "
        ...
        do {
            for (struct addrinfo *walk = res; walk != nullptr; walk = walk->ai_next)
            {
                ...
                if (Handle < 0)
                {
                    std::cout << "BinaryInterfaceClien::Connect: Attempt failed. Errno was: "
                              << std::to_string(Error) << ": " << std::string(strerror(Error));
                    continue;
                }
        ...
        if (Handle == -1)
        {
            throw std::runtime_error("BinaryInterfaceClien::Connect: Error during connection. Last errno was: "
                    + std::to_string(Error)  + ": " + std::string(strerror(Error)));
        }
        ...
        if (rc < 0) {
            throw std::runtime_error("BinaryInterfaceClien::Connect: Error during setsockopt: TCP_NODELAY");
        }
